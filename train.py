...
class Supervision_Train(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        if self.config.weights_name == 'Model_v2':
            seg_pre = self.net(x)[-1]
        elif self.config.weights_name == 'model_v3':
            seg_pre = self.net(x)[-1]
        elif self.config.weights_name == 'Ablation_3':
            seg_pre = self.net(x)[-1]
        else:
            seg_pre = self.net(x)
        return seg_pre
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.config.weights_name == 'ABCNet':
            predictions = self.net(img)
            loss = self.loss(predictions[0], mask) + self.loss(predictions[1], mask) + self.loss(predictions[2], mask) 
            prediction = predictions[0]
        elif self.config.weights_name == 'Model_1':
            predictions = self.net(img)
            loss = self.loss(predictions[0], mask) + self.loss(predictions[1], mask) + self.loss(predictions[2], mask) + self.loss(predictions[3], mask)
            prediction = predictions[-1]
        elif self.config.weights_name == 'Model_v2':
            predictions = self.net(img)
            loss = self.loss(predictions[0], mask) + self.loss(predictions[1], mask) + self.loss(predictions[2], mask) + self.loss(predictions[3], mask)
            prediction = predictions[-1]
        elif self.config.weights_name == 'model_v3':
    ...
    def training_epoch_end(self, outputs):
        if 'whubuilding' in self.config.log_name:

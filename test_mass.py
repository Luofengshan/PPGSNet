...
def main():
    ...
    with torch.no_grad():
        ...
        for input in tqdm(test_loader):
            ...
            for i in range(raw_predictions.shape[0]):
                ...
                # print(mask.shape)
                if 'gt_semantic_seg' in input.keys():
                    evaluator.add_batch(pre_image=raw_mask, gt_image=masks_true[i].cpu().numpy())
                mask_name = image_ids[i]
                results.append((raw_mask, str(args.output_path / mask_name), args.rgb))
    t0 = time.time()
    ...
    print('F1:{}, IOU:{}, OA:{}, P:{}, R:{}'.format(np.nanmean(f1_per_class[0]), np.nanmean(iou_per_class[0]), OA,
                                                    np.nanmean(precision[0]), np.nanmean(recall[0])))
